		...
		if (node instanceof DefaultMutableTreeTableNode) {
			...
			if (defNode.getUserObject() instanceof FoundElementToDelete) {
				...
				if (column == Constants.DeleteColumns.ELEMENT.index) 	{
					if(!(element.getTableDescription().compareTo(Constants.TitlesTabs.REACTIONS.description) ==0  && element.getCol()==Constants.ReactionsColumns.REACTION.index)) toBeDisplayed +="("+MainGui.printCellHeader(element) +") ";
					...
					if(toBeDisplayed.toString().compareTo(Constants.DeleteActions.ASSIGN_NEW_VALUE.description)==0) {
					...
					if (element.getActionToTake().compareTo(Constants.DeleteActions.ASSIGN_NEW_VALUE.description)!= 0) toBeDisplayed ="";  
					else {
						...
						if(parent!=null) {
							if (parent.getUserObject() instanceof FoundElement) {
								...
								if(element.getNewValue().length()==0) {
									...
									if(typeParentToDelete.compareTo(Constants.TitlesTabs.SPECIES.description)==0) {	toBeDisplayed = MainGui.species_defaultInitialValue;	}
									else if(typeParentToDelete.compareTo(Constants.TitlesTabs.COMPARTMENTS.description)==0) {toBeDisplayed = MainGui.compartment_defaultInitialValue;	}
									else if(typeParentToDelete.compareTo(Constants.TitlesTabs.EVENTS.description)==0) {	toBeDisplayed = "";	}
									else if(typeParentToDelete.compareTo(Constants.TitlesTabs.GLOBALQ.description)==0) {	toBeDisplayed = MainGui.globalQ_defaultValue_for_dialog_window;	}
									else if(typeParentToDelete.compareTo(Constants.TitlesTabs.REACTIONS.description)==0) {	toBeDisplayed = "";		}
			...
			if (node instanceof DefaultMutableTreeTableNode) {
				...
				if (defNode.getUserObject() instanceof FoundElementToDelete) {
					...
					if (column == Constants.DeleteColumns.ELEMENT.index) 	{
						...
						if(value.toString().startsWith(Constants.DeleteActions.ASSIGN_NEW_VALUE.description)) {
							element.setActionToTake(Constants.DeleteActions.ASSIGN_NEW_VALUE.description);
						} 
					}
					else if (column == Constants.DeleteColumns.NEW_VALUE.index)  {
      ...
      @Override
        public boolean isCellEditable(Object node, int col)
        {
    	  	...
    	  	if (node instanceof DefaultMutableTreeTableNode) {
				...
				if (defNode.getUserObject() instanceof FoundElementToDelete) {
					FoundElementToDelete element = (FoundElementToDelete) defNode.getUserObject();
					if(element.getTableDescription().compareTo(Constants.TitlesTabs.REACTIONS.description)==0 && element.getCol()==Constants.ReactionsColumns.REACTION.index) return false;
					if(element.equals(DeleteFrame.rootElementToDelete)) return false;
					
					if(((FoundElementToDelete)((DefaultMutableTreeTableNode) node).getParent().getUserObject()).getTableDescription().compareTo(Constants.TitlesTabs.COMPARTMENTS.description)==0) {
						if(element.getTableDescription().compareTo(Constants.TitlesTabs.SPECIES.description)==0) {
							return false;
						}
					}
					
					if(element.getTableDescription().compareTo(Constants.TitlesTabs.EVENTS.description)==0 && 
					...
					if(element.getActionToTake().compareTo(Constants.DeleteActions.ASSIGN_NEW_VALUE.description)==0) return true;
					return false;
				}
			}
    	  	return true;
