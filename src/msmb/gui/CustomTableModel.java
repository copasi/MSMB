		             ...
		             if(MainGui.isCellWithMultipleView(model.getTableName(), row, col)) {
		         	   ...
		         	   if(model.getTableName().compareTo(Constants.TitlesTabs.REACTIONS.description)==0) {
		         		  ...
		         		  goToDefinition.addActionListener(new ActionListener() {
		         			  @Override
		         			  public void actionPerformed(ActionEvent e) {
		         				    MutablePair<Integer, Integer> index = model.getFunctionDefinitionIndex();
...
*/
	...
	public void editingStopped(ChangeEvent e) {
        ...
        TableCellRenderer cellRender = nameColumn.getCellRenderer();
     	...
     	TableCellEditor cellEditor = nameColumn.getCellEditor();
        if(cellEditor instanceof ExpressionsCellEditor) {
     		((ExpressionsCellEditor) cellEditor).stopTimerAutocompletion();
     	};
		...
		try{
			...
			if(renderer instanceof EditableCellRenderer) {
				if(this.model.disabledCell.contains(rowIndex+"_"+vColIndex)) {
					...
					if(this.model.getTableName().compareTo(Constants.MultistateBuilder_QUANTITIES_description)==0) c.setForeground(Color.BLACK);
