...
public class MultiModel {
	...
	public void addNamedElement(String name, Integer tableIndex) throws Exception { 
		...
		if(where != null) { 
			if(where.contains(tableIndex)) throw new Exception(); //already defined in that table
		} else {
	...
	public String mergeMultistateSpecies(int indexOld, MultistateSpecies newSp) throws Exception {
		int ind = -(speciesDB.addChangeSpecies(-1,new String(),newSp.printCompleteDefinition(),newSp.getInitialQuantity_multi(),newSp.getType(), newSp.getCompartment(), newSp.getExpression(),false, new String(),true,true));
	...
	public int findMetabolite(String name) throws Exception {
		return findMetabolite(name, false);
	}
	
	private int findMetabolite_key(String name) throws Exception {
		return findMetabolite(name, true);
	}
	...
	private void fillCopasiDataModel(CustomTableModel tableReactionmodel, ProgressBarFrame progressBarFrame) throws Exception {
		...
		/*long size = CCopasiRootContainer.getDatamodelList().size();
	...
	private CReaction generateKineticLaw(CReaction reaction, int i, Vector expandedReaction, String rateLaw, String kineticType, CModel model) throws Exception {
				...
				do{
					try {
							...
							if(index_metab == -1) {
								for(int g = 0; g < expandedVectorElements.size(); g++) {
									...
									if(current.contains(actualModelParameter)) {
										index_metab = this.findMetabolite(current);
									}
								}
							}
						...
						case CFunctionParameter.PRODUCT:  
							...
							index_metab = this.findMetabolite(actualModelParameter);
							if(index_metab == -1) {
								for(int g = 0; g < expandedVectorElements.size(); g++) {
									String current = (String)(expandedVectorElements.get(g));
									if(current.contains(actualModelParameter)) {
		...
		if(f instanceof CFunction && !foundUserDefFunction && !reaction.getFunction().getObjectName().contains("action") &&
		...
		for(int r = 0; r < prod.size(); r++ ) {
			...
			} else reaction.addParameterMapping("product",  metab.getKey());
		...
		for(int i = 0; i < species.size(); i++) {
			...
			if(s!= null){
				if(s instanceof Species && !(s instanceof MultistateSpecies)) {
					...
					int type = s.getType();
					CMetab m;
					int index_metab = this.findMetabolite(name);
					...
					if(initialExpression.length()>0 ) {
						if(initialExpression.toLowerCase().compareTo(Constants.NAN_STRING)!=0) m.setInitialExpression(buildCopasiExpression(initialExpression, false,true));
					...
					for(int j = 0; j < expanded.size(); j++) {
						...
						String comp = single.getCompartment();
						double initial_conc = 0.0;
						if(MainGui.quantityIsConc) {
							try{
	...
	private void fillCopasiDataModel_globalQ_fixed() {
		...
		while(it.hasNext()) {
			...
			try{
				...
				if(MainGui.DEBUG_SHOW_PRINTSTACKTRACES) ex.printStackTrace();
				if(g.getInitialValue().toLowerCase().compareTo(Constants.NAN_STRING)==0) {
	...
	private Vector<String> parseExpressionAssignment(Vector<String> expression, int expansionActionWithVolume) throws Exception {
		...
		for(int i = 0; i < expression.size(); i++) {
			...
			Vector<String> couple = new Vector<String>();
	   		...
	   		if(target.contains("(")) { //is a species and is multistate
			    	...
			    	int index_metab = this.findMetabolite(complete_name);
					CMetab metab = model.getMetabolite(index_metab);
					...
					int index = this.findMetabolite(target);
				...
				//NO... ADESSO VARIA A SECONDA DI QUELLO CHE SI DICE IN GENERALE... always concentration because the events hold the particle number automatically
	...
	//	REAZIONE 52
		...
		for(int i = 0; i < subs.size(); i++) {
			...
			if(species.contains("(")) {//FIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIX
...
//------------------------------------------------------------------
           ...
           //--  
			...
			}	 r_string += sub.getMetabolite().getObjectName() + " + ";*/
			 r_string += cleanName  + " + ";
            }
	...
	private String buildMRKineticLaw_fromCopasiFunction(CReaction reaction, boolean isBackward) throws Exception {
		...
		for(int i = 0; i < param.size(); i++) {
			...
			if(currentKey.contains("Value")) {
				...
				if(complete_name_species.contains("(")) complete_name_species = complete_name_species.substring(0,complete_name_species.indexOf("("));
				ret += CellParsers.cleanName(complete_name_species) + ",";
...
//------------------------------------------------------------------
		...
		while(st_elem.hasMoreTokens()) {
			...
			if(elem.contains("[")) {
				...
				while(st_vector.hasMoreTokens()) {
					//for now I ignore the fact that metabolites can be in different compartments
					//I just keep track of the LAST "Vector=" element and I use it as the element in my expression string
					//for Metabolites there are 2 "Vector=[]": the first is the compartment the secon the metabolite... for now I'm extracting only the name of the species
					String sub_elem = st_vector.nextToken();
					if(sub_elem.contains("Vector")) {
	...
	public String reprintExpression(String expr, boolean forceExpansion_elements) throws Exception {
		...
		try{
			...
			ExpressionVisitor vis = new ExpressionVisitor(new ArrayList<String>(),this,forceExpansion_elements);
			root.accept(vis);
			if(vis.getExceptions().size() == 0) {
	...
	private String buildMRAssigments_fromCopasiExpr(CEvent event) {
		...
		for(int i = 0; i < assignSize; i++) {
			...
			try { //IT CAN BE SOMETHING DIFFERENT FROM A METAAAAAAAAAAAAAAAAAAAAAAAAAAAABOLITE
				...
				int index = this.findMetabolite(element.getTargetKey(),true);
		    	if(index!= -1) { //species
		    		ret += model.getMetabolite(index).getObjectName();
		    	} else { 
	...
	public boolean updateGlobalQ(Integer nrow, String name, String value, String type, String expression, String notes) throws Exception {
		...
		if(expression.toLowerCase().compareTo(Constants.NAN_STRING)==0) {
			  DebugMessage dm = new DebugMessage();
				dm.setOrigin_table(Constants.TitlesTabs.GLOBALQ.description);
		...
		if(value.toLowerCase().compareTo(Constants.NAN_STRING)==0) {
			  DebugMessage dm = new DebugMessage();
				dm.setOrigin_table(Constants.TitlesTabs.GLOBALQ.description);
	...
	public Vector<Vector> updateSpecies(Integer nrow, String name, String initialQ, String type, String compartment, String expression, boolean autoFill, String notes, boolean autoMergeSpecies) throws Exception{
		...
		if(initialQ.toLowerCase().compareTo(Constants.NAN_STRING)==0) {
			  DebugMessage dm = new DebugMessage();
				dm.setOrigin_table(Constants.TitlesTabs.SPECIES.description);
